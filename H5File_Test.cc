...
void H5File_Test::testTrainFormat() {
  ...
  TimeProfiler p("testTrainFormat");
  p.open();
  p.startPeriod("allTest");
	...
	// auto
	  ...
	  // last parameter is imageCount
	  // uint64_t trainLength = fillTrainBuffer(buffer, bufLen, dataset, i, i * 2 + 4);
	  // make it bigger: from ~50MB to ~1GB
	  uint64_t trainLength = fillTrainBuffer(buffer, bufLen, dataset, i, 20 *(i * 2 + 4));
  ...
  p.stopPeriod("allTest");
  p.close();

  TimeDuration testTime = p.getPeriod("allTest").getDuration();
  std::cerr << " *** testTrainFormat run time:"  << testTime << std::endl;
}
...
karabo::io::h5::Format::Pointer H5File_Test::trainFormatImages(const Hash& dataset) {
...

    {
        ...
        Hash c(
               ...
               "type", "PTR_UINT16",
               "key", "images.image"
	       // (un)comment to enable/disable compression in the trainFormat test
	       //,
               //"compressionLevel",4
               );
